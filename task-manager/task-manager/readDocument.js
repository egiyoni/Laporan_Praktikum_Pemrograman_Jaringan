...
async function main() {
  try {
    ...
    // Mencari satu dokumen dalam koleksi 'pengguna' berdasarkan nama 'Rijal'.
    const byNama = await db.collection("pengguna").findOne({ nama: "Rijal" });
    // Mencari satu dokumen dalam koleksi 'pengguna' berdasarkan ID objek tertentu.
    const byObjectID = await db
      .collection("pengguna")
      .findOne({ _id: new ObjectId("654345a430761f70fbd498fa") });
